...
Momentan wurde die Applikation hauptsächlich auf Android Smartphones mit Google Chrome getestet. Da es sich um eine Webapplikation handelt ist der Betriebsystem von kleinere Bedeutung. Wichtig ist, dass WebRTC von den Browser unterstützt wird.
\\
...
\begin{itemize}
	\item Die vorher generierte Mobile-Zertifikat Datei (\seeref{kap:zertifikategen}) auf dem Smartphone kopieren. Das kann via USB oder E-Mail erfolgen. Wichtig ist, dass die Datei auf der lokale Speicher des Mobilgeräts vorhanden ist.
...
\textbf{Schritt 2: Einbindung an der Homescreen}
\\
...
\\
\textit{* SERVER -> Die IP Adresse oder Hostname der Server.}
\\
\textit{** id=1 -> Diese ID definiert welche Einwohner sich gerade Anmeldet. Welche Einwohner zu welchen ID gehört, wird in den Management Tool definiert. Wie in den \ref{kap:clientapp} bereits erwähnt, diese ist keine Endgültige Lösung und ist nur für Vorführzwecke da.}
\\
\\
Die Webapplikation besitzt über ein \textit{Manifest-File} welche die Applikation als Standalone App konfigurieren kann. Um das zu aktivieren muss die Webseite zu den Homescreen hinzugefügt werden. Um das zu machen auf das Overflow-Menü (drei kleine Punkte oben rechts) tippen und \textit{Zum Startbildschirm hinzufügen} auswählen.
\\
\\
