...
Die Hardware ist nun vollständig und dient als Basis für die Entwicklung der Softwarekomponenten die für das System notwendig sind. Die verwendete Software und Programmiersprachen wurden in dem Fachmodul evaluiert.
...
\begin{figure}[htb!]
	\begin{center}
		...
		\caption[STUN Server]{STUN Server (Quelle: https://realtimecommunication.wordpress.com/2015/05/29/crack-the-nat/)}
		\label{fig:stun}
	\end{center}
\end{figure}
...
\\
Aus Zeitgründen könnten wir die Anmeldeseite für den Client Applikation nicht vervollständigen. Momentan wird die Identität der Anwender lediglich über eine GET Variable definiert. (\seeref{ssec:clientappinst}). Eine Anmeldeseite welche die Identität der Anwohner überprüft ist für das Endprodukt natürlich enorm wichtig. Die jetzige Lösung stellt eine grosse Sicherheitslücke dar und ist nur für Test- und Vorführzwecke da. Eine Login-Page wurde aber vom Anfang an vorgesehen und die Implementierung kann nun ohne Anpassung der Bestehende Sourcecode vorgenommen werden.
...
\label{kap:remote}
Die Remote Verbindung auf den \gls{turklingelanlage} ist eine Feature die leider aus Zeitgründen nicht implementiert werden konnte. 
\\
Stellt man sich folgende Situation vor: Man sei unterwegs und niemand steht in der Wohnung, nun soll der Briefträger ein Päckchen liefern. Hier kommt die Remote Funktionalität im Spiel. Über den \gls{clientapp} könnte die Indentität des Person welches vor dem Eingang steht überprüft werden, und allenfalls könnte die Tür geöffnen werden. Bei eine zukünftige implementation des Feature soll besonders Wert auf die Sicherheit gelegt werden. Grund dafür ist dass den die Verbindung zwischen die Komponenten das Lokalen Netz verlassen würde. Diese würde die Anlage an mögliche bösartige Angriffe aussetzen. 
\\
Diese Funktionalität, welches mit eine Analoge System kaum realisierbar ist, würde die Vorteile des Digitalisierung noch deutlicher hervorheben. Auch aus der Marketingssicht stellt eine solche Funktionalität ein erhebliche Mehrwert dar. 
