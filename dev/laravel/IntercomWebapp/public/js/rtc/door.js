 ...
 * UI SELECTORS
 */
var localAudio = document.querySelector('#localAudio'); // Currently not used
...
function startWebSocket()
{
    ...
    socketConn = new WebSocket(socketProtocol +"://"+signalingSrvAddr+":"+signalingSrvPort); // Port and Address variables generated by php
    ...
    socketConn.onmessage = function (msg)
    {
        ...
        switch(data.type) {
            ...
            case SPEAKER_STATUS:
            {
                switchSpeakerStatus(data.value);
            }
            default:
                console.log(data);
                break;
...
function setupRTC(value) {
    ...
    else
    {
        ...
        navigator.webkitGetUserMedia({ video: true, audio: true }, function (myStream) {
            ...
            //using Google public stun server (this is used only if needed)
            var configuration = {
...
function handleLeave()
{
    ...
    switchSpeakerStatus(0); // Turn speaker off
...
// INIT
startWebSocket(); // For the signaling server
startLocalSocket(); // For the speaker controller

// TURN SPEAKER ON & OFF ////////////////////////////////
// Sends a message to the local service that turns the speaker on and off
...
{
    console.log("Switching speaker status: " + value);
    localSocketConn.send(JSON.stringify({type: "SPEAKER_STATUS", value: value}));
}
